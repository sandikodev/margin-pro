...
// --- Insert/Update Schemas for DB Operations ---

// Business insert schema (for creating new businesses)
export const insertBusinessSchema = businessSchema.omit({ id: true });
export type InsertBusinessSchema = z.infer<typeof insertBusinessSchema>;

// Business update schema (for updating existing businesses)
export const updateBusinessSchema = businessSchema.partial().required({ id: true });
export type UpdateBusinessSchema = z.infer<typeof updateBusinessSchema>;

// Project insert/update schemas
...
// Liability insert/update schemas
export const insertLiabilitySchema = liabilitySchema.omit({ id: true });
export const updateLiabilitySchema = liabilitySchema.partial().required({ id: true });

// Cashflow insert/update schemas
export const insertCashflowSchema = cashflowSchema.omit({ id: true });
export const updateCashflowSchema = cashflowSchema.partial().required({ id: true });
