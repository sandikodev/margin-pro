...
import { Menu, RefreshCw, Volume2, User as UserIcon, ChevronRight, ArrowLeft, PenLine, Search, History } from 'lucide-react';
import { CURRENCIES } from '@/lib/client/constants';
import { Project, BusinessProfile, User } from '@shared/types';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  fetchLiveRates, isRefreshingRates, isSpeaking, handleAudioSummary, setActiveTab, credits, activeBusiness, user,
  ...
  const getPageTitle = () => {
    ...
    switch (activeTab) {
      case 'home': return (activeBusiness?.name || 'Merchant') + ' ✨';
  ...
  const getSubTitle = () => {
    ...
    if (activeTab === 'home') return 'Selamat Datang,';
    ...
    <header className="h-16 lg:h-20 bg-white/90 backdrop-blur-md border-b border-slate-200/60 px-4 lg:px-10 flex items-center justify-between z-30 shrink-0 sticky top-0 transition-all">
      ...
      <div className="flex items-center gap-2 lg:gap-4 flex-shrink-0">
        <div className="hidden sm:flex items-center gap-2 mr-2">
          <button className="w-9 h-9 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-white transition-all active:scale-90">
            <Search className="w-4 h-4" />
          </button>
          <button className="w-9 h-9 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 hover:text-indigo-600 hover:bg-white transition-all active:scale-90 relative">
          ...
          >
            ...
            <div className="hidden lg:flex flex-col text-left">
              <span className={`text-[9px] font-black uppercase tracking-tight leading-none ${activeTab === 'profile' ? 'text-white' : 'text-slate-900'} truncate max-w-[80px]`}>
                {user?.name || activeBusiness?.name || 'User'}
              </span>
