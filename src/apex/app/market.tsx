...
import { FilterModal, SortOption } from '@/core/ui/modals/FilterModal';
import { MarketItemCard } from '@/core/ui/features/market/MarketItemCard';
...
export default function MarketPage() {
    const { addProject, activeBusinessId, setActiveTab, setActiveProjectId } = useOutletContext<DashboardOutletContext>();
    const { deductCredits } = useMarketplace();
    const { formatValue } = useCurrency();
    const { showToast } = useToast();

    // --- LOGIC FROM MarketPage ---
    const handleBuyItem = async (item: MarketplaceItem) => {
        ...
        setActiveTab('calc'); // Assuming this navigates to /app/project via activeTab logic
        showToast("Template berhasil dibeli", "success");
    };

    // --- LOGIC FROM MarketplaceView ---
    ...
    const filteredItems = useMemo(() => {
        let result = INITIAL_MARKETPLACE.filter(item =>
    ...
    }, [searchTerm, activeSort, activeCategory]);
