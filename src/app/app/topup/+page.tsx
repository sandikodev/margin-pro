...
export const TopUpPage: React.FC = () => {
   ...
   const { formatValue, credits: currentCredits, setActiveTab } = ctx;
   ...
   return (
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-40 max-w-xl mx-auto">
         ...
         <div className="px-2 pt-2">
            <BentoCard className="bg-slate-900 border-none p-8 text-white group">
               ...
               {PACKAGES.map((pkg) => {
                  ...
                  return (
                     <BentoCard
                        ...
                        noPadding
                        className={`transition-all duration-300 cursor-pointer active:scale-[0.98] group p-1
                     ${isSelected ? 'bg-gradient-to-r from-indigo-500 to-violet-500 shadow-xl shadow-indigo-200 border-transparent' : 'bg-white hover:bg-slate-50'}`}
                     >
                        <div className={`h-full bg-white rounded-[2.2rem] p-6 relative overflow-hidden`}>
                           {pkg.popular && (
