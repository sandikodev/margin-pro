...
export const PlatformsTab: React.FC<PlatformsTabProps> = ({ addAuditLog }) => {
    ...
    const handleSavePlatform = async (id: string) => {
        ...
        try {
            ...
            await (api.admin.platforms[':id'] as never).$put({
                param: { id },
                json: platformBuffer
            });
                                    ...
                                    if (confirm(`Apply ${val * 100}% commission to ALL platforms?`)) {
                                        ...
                                        try {
                                            await Promise.all(Object.keys(platforms).map(id =>
                                                // @ts-expect-error - RPC inference
                                                (api.admin.platforms[':id'] as never).$put({ param: { id }, json: { ...platforms[id as Platform], defaultCommission: val } })
                                            ));
                                            await refreshConfigs();
                                ...
                                <div key={field.key} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm group-hover:border-indigo-50 transition-colors">
                                    ...
                                    {editingPlatform === id ? (
                                        <input
                                            ...
                                            disabled={isSaving}
                                            className="w-full text-lg font-black text-indigo-600 outline-none bg-transparent disabled:opacity-50"
                                            value={platformBuffer[field.key as keyof PlatformConfig] as number || ''}
