...
import { DashboardSectionHeader } from '@/core/ui/design-system/SectionHeader';
...
export const PricingStrategySection: React.FC<PricingStrategySectionProps> = ({
   activeProject, activeBusiness, updateProject, formatValue, prodConfig, totalEffectiveCost, onSimulate
}) => {
   const strategy = activeProject.pricingStrategy || 'markup';

   // Use project override or business default
   const targetMargin = activeProject.targetMargin || activeBusiness?.targetMargin || 20;

   // Suggested profit based on margin percentage if targetNet is 0
   const isUsingMarginSuggestion = activeProject.targetNet === 0;
   const suggestedProfit = isUsingMarginSuggestion
      ? totalEffectiveCost * (targetMargin / 100 / (1 - targetMargin / 100))
      : activeProject.targetNet;

   const baseProfitCompetitor = (activeProject.competitorPrice || 0) - totalEffectiveCost;
   const basePrice = totalEffectiveCost + suggestedProfit;

   const totalSimulatedProfit = strategy === 'competitor'
      ? baseProfitCompetitor * prodConfig.targetUnits
      : suggestedProfit * prodConfig.targetUnits;
            ...
            <GradientCard className="p-6 lg:p-8 flex flex-col justify-center min-h-[180px] group transition-all hover:scale-[1.01]">
               <div className="relative z-10 space-y-2">
                  <div className="flex items-center justify-between gap-2 mb-2">
                     ...
                     {strategy === 'markup' && isUsingMarginSuggestion && (
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-white/10 rounded-full backdrop-blur-md border border-white/10">
                           <span className="w-1 h-1 bg-emerald-400 rounded-full animate-pulse"></span>
                           <span className="text-[8px] text-indigo-100 font-bold uppercase tracking-tight">Auto: {targetMargin}% Margin</span>
                        </div>
                     )}
                  </div>
                  ...
                  <div className="relative border-b-2 border-indigo-400/30 pb-2 group-focus-within:border-indigo-400 transition-colors">
                     ...
                     <input
                        ...
                        placeholder={strategy === 'markup' ? Math.round(suggestedProfit).toString() : "0"}
                     />
                  </div>
                  <p className="text-[9px] text-indigo-200/60 font-medium leading-relaxed">
                     {strategy === 'markup'
                        ? isUsingMarginSuggestion
                           ? `Menggunakan target margin bisnis Anda (${targetMargin}%). Masukkan nominal jika ingin target profit tetap.`
