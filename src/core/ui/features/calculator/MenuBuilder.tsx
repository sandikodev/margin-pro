...
import { OperationalContext } from '@/core/ui/calculator/OperationalContext';
import { CostList } from '@/core/ui/calculator/CostList';
import { PricingStrategySection } from '@/core/ui/calculator/PricingStrategySection';
import { calculateTotalHPP } from '@/shared/utils/business-logic';
...
export const MenuBuilder: React.FC<MenuBuilderProps> = ({
  activeProject, activeBusiness, updateProject, createNewProject, deleteProject, formatValue, onSimulate
}) => {
    ...
    <div className="space-y-6 lg:space-y-8 animate-in fade-in duration-500 relative pb-32">
      ...
      <div className="lg:hidden fixed bottom-28 right-6 z-50 flex flex-col items-end gap-4 pointer-events-none">
        <div className={`flex flex-col items-end gap-3 transition-all duration-300 ${isMenuOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-10 pointer-events-none'}`}>
          ...
          {fabMode === 'main' ? (
            <>
              <button onClick={() => setFabMode('export')} className="flex items-center gap-3 group mr-1">
                <span className="bg-white text-slate-700 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl shadow-md border border-slate-100 scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100 transition-all origin-right">Export</span>
                <div className="w-12 h-12 bg-white text-slate-600 border border-slate-200 rounded-full shadow-xl flex items-center justify-center group-active:scale-90 transition-transform"><Share2 className="w-5 h-5" /></div>
              </button>
              <button onClick={handleDelete} className="flex items-center gap-3 group mr-1">
                <span className="bg-white text-slate-700 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl shadow-md border border-slate-100 scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100 transition-all origin-right">Hapus</span>
                <div className="w-12 h-12 bg-rose-600 text-white rounded-full shadow-xl shadow-rose-600/40 flex items-center justify-center group-active:scale-90 transition-transform"><Trash2 className="w-5 h-5" /></div>
              </button>
              <button onClick={handleCreate} className="flex items-center gap-3 group mr-1">
                <span className="bg-white text-slate-700 text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-xl shadow-md border border-slate-100 scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100 transition-all origin-right">Menu Baru</span>
                <div className="w-12 h-12 bg-indigo-600 text-white rounded-full shadow-xl shadow-indigo-600/40 flex items-center justify-center group-active:scale-90 transition-transform"><FilePlus className="w-5 h-5" /></div>
              </button>
            </>
          ) : (
            <>
              ...
              <button onClick={handleCopyJSON} className="flex items-center gap-3 group mr-1 animate-in slide-in-from-right-4 duration-300 delay-150">
                ...
                <div className="w-12 h-12 bg-indigo-500 text-white rounded-full shadow-xl flex items-center justify-center">{copySuccess ? <Check className="w-5 h-5" /> : <Copy className="w-5 h-5" />}</div>
              </button>
