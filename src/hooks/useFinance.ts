...
export const useFinance = (
  ...
  initialFinance?: { liabilities: Liability[], cashflow: CashflowRecord[] },
  initialSettings?: { monthlyFixedCost: number, currentSavings: number }
) => {
  ...
  const { data: liabilities = [] } = useQuery({
    ...
    enabled: !!businessId,
    initialData: initialFinance?.liabilities
  });
  ...
  const { data: cashflow = [] } = useQuery({
    ...
    enabled: !!businessId,
    initialData: initialFinance?.cashflow
  });
